...
void DockXP(int client, int xp_penalty, bool ban) {
    ...
    if(GetClientAuthId(client, AuthId_Steam3, steam_buff, STEAM_ID_LEN)) {
        Format(query, sizeof(query), "UPDATE pro_xp SET xp = GREATEST(xp-%i, 0) WHERE steamid = '%s'", xp_penalty, steam_buff);
        SQL_LockDatabase(DB);
    ...
    if(xp_penalty > 0) {
        ...
        CPrintToChat(client, "{red}You have been docked %s xp for cheating.", penalty);
    }
    if(ban) {
...
public void Setup() {
    ...
    if(StrEqual(db_ident, "mysql", false)) {
        SQL_FastQuery(DB, "CREATE TABLE IF NOT EXISTS `pro_xp` (  `id` int(11) NOT NULL,  `steamid` varchar(32) DEFAULT NULL,  `name` varchar(64) DEFAULT NULL,  `xp` int(11) NOT NULL DEFAULT '0',  `sxp` int(11) DEFAULT '0',  `steam3` varchar(32) DEFAULT NULL,  `deaths` int(11) NOT NULL DEFAULT '0',  `kills` int(11) NOT NULL DEFAULT '0',  `hits` int(11) NOT NULL DEFAULT '0',  `shots` int(11) NOT NULL DEFAULT '0',  `jumpshots` int(11) NOT NULL DEFAULT '0',  `noscopes` int(11) NOT NULL DEFAULT '0',  `knife_kills` int(11) NOT NULL DEFAULT '0',  `headshots` int(11) NOT NULL DEFAULT '0',  `damage` bigint(11) NOT NULL DEFAULT '0',  `grenade_kills` int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARSET=latin1;");
        SQL_FastQuery(DB, "ALTER TABLE `pro_xp` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `steamid_unique` (`steamid`);");
        SQL_FastQuery(DB, "ALTER TABLE `pro_xp` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");
    } else if (StrEqual(db_ident, "sqlite")) {
        SQL_FastQuery(DB, "CREATE TABLE IF NOT EXISTS 'pro_xp' ('id' INTEGER NOT NULL, 'steamid' TEXT NOT NULL UNIQUE, 'name' TEXT NOT NULL, 'xp' INTEGER NOT NULL DEFAULT 0, 'sxp' INTEGER NOT NULL DEFAULT 0, 'steam3' TEXT NOT NULL UNIQUE, 'deaths' INTEGER NOT NULL DEFAULT 0, 'kills' INTEGER NOT NULL DEFAULT 0, 'hits' INTEGER NOT NULL DEFAULT 0, 'shots' INTEGER NOT NULL DEFAULT 0, 'jumpshots' INTEGER NOT NULL DEFAULT 0, 'noscopes' INTEGER NOT NULL DEFAULT 0, 'knife_kills' INTEGER NOT NULL DEFAULT 0, 'headshots' INTEGER NOT NULL DEFAULT 0, 'damage' INTEGER NOT NULL DEFAULT 0, 'grenade_kills' INTEGER NOT NULL DEFAULT 0, PRIMARY KEY('id' AUTOINCREMENT));");
    } else if (StrEqual(db_ident, "pgsql")) {
        SQL_FastQuery(DB, "CREATE TABLE IF NOT EXISTS 'pro_xp' (id integer NOT NULL, steamid character varying(32) NOT NULL, name character varying(128) NOT NULL, xp integer DEFAULT 0 NOT NULL, sxp integer DEFAULT 0 NOT NULL, steam3 character varying(32) NOT NULL, deaths integer DEFAULT 0 NOT NULL, kills integer DEFAULT 0 NOT NULL, hits integer DEFAULT 0 NOT NULL, shots integer DEFAULT 0 NOT NULL, jumpshots integer DEFAULT 0 NOT NULL, noscopes integer DEFAULT 0 NOT NULL, knife_kills integer DEFAULT 0 NOT NULL, headshots integer DEFAULT 0 NOT NULL, damage integer DEFAULT 0 NOT NULL, grenade_kills integer DEFAULT 0 NOT NULL);");
        ...
        SQL_FastQuery(DB, "ALTER TABLE ONLY pro_xp ADD CONSTRAINT pro_xp_steamid_key UNIQUE (steamid);");
    } else {
...
public Action EventPlayerHurt(Handle event, const char[] name, bool dontBroadcast) {
    ...
    if(victim == 0 || attacker == 0 || attacker == victim || IsFakeClient(attacker) || levels[attacker] == 0) {
        return;
    }
    
    int damage = 0;
    bool killshot, headshot, noscope, jumpshot, grenade = false;
    int xp = CalculateXP(event, damage, killshot, headshot, noscope, jumpshot, grenade);
    Forward_OnGainXP(attacker, victim, xp, damage, killshot, headshot, noscope, jumpshot, grenade);
}
...
public void PlayerJoinSetup(int client) {
    ...
    char query[200];
    Format(query, sizeof(query), "SELECT xp, deaths, kills, hits, shots, jumpshots, noscopes, knife_kills, headshots, damage, grenade_kills FROM pro_xp WHERE steamid = '%s'", steam_ids[client]);
...
public void PlayerJoinGetXPCallback(Database db, DBResultSet result, const char[] error, int client) {
    ...
    if(result != null && result.FetchRow()) {
        ...
        #endif
        player_stats[client].join(exp, deaths, kills, hits, shots, jumpshots, noscopes, knife_kills, headshots, damage, grenade_kills);
        ...
        Format(query, sizeof(query), "INSERT INTO pro_xp (steamid, name, xp, sxp, steam3) VALUES('%s', '%s', 0, 0, '%s')", steam_ids[client], client_name, temp_Steam64);
...
public void UpdateDatabaseXP(int client) {
    ...
    player_stats[client].update(DB, steam_ids[client]);
}
...
public int CalculateXP(Handle event, int &damage, bool &killshot, bool &headshot, bool &noscope, bool &jumpshot, bool &grenade) {
    int attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
    int victim = GetClientOfUserId(GetEventInt(event, "userid"));
    ...
    // if(GetConVarInt(funround) == 1 && GetConVarInt(fun_ammo) == 1) {
    //     bonus_mult *= 0.6; // lower the xp bonus due to infinite ammo
    // }
...
public void SetLevelTag(int client) {
    if(!IsClientInGame(client) || IsFakeClient(client) || levels[client] == 0) {
        return;
    }
    char buff[24];
    if(levels[client] >= LEVEL_CAP) {
        #if LEVEL_CAP_PREFIX
            Format(buff, sizeof(buff), "%sLvl %i%s", LEVEL_CAP_TAG_PREFIX, levels[client], LEVEL_CAP_TAG_SUFFIX);
        #else
            Format(buff, sizeof(buff), "%s", LEVEL_CAP_TAG);
        #endif
        CS_SetMVPCount(client, LEVEL_CAP);
    } else {
        Format(buff, sizeof(buff), "Lvl %i", levels[client]);
    }
